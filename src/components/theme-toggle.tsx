...
console.log(resolvedTheme)
  return !mounted ? <Button variant="ghost" size="icon" className="h-4 w-4 px-2 py-1.5"/> : (
        ...
        <Button variant="ghost" size="icon" className={cn("group h-fit w-fit gap-2 px-2 py-1.5 hover:bg-stone-200 active:bg-stone-300",
          ...
          {theme === "system" &&
            <div className="relative h-5 w-5">
              ...
              {resolvedTheme === "dark" &&
                <Icons.theme.dark className="absolute animate-daleyShow opacity-0 transition-all group-hover:hidden"/>}
              {resolvedTheme === "light" &&
                <Icons.theme.light className="absolute animate-daleyShow opacity-0 transition-all group-hover:hidden"/>}
            </div>
          }
